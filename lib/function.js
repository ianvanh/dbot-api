...
function liric(name){
	return new Promise(async(resolve, reject) => {
   		axios.get('https://www.musixmatch.com/search/' + name)
   		.then(async({ data }) => {
		   ...
		   result.liric = $$(b).find('span > p > span').text() +'\n' + $$(b).find('span > div > p > span').text()
		   })
	   })
	   resolve(result)
   })
...
function wallpaper(name) {
	return new Promise((resolve, reject) => {
		axios.get('https://www.wallpaperflare.com/search?wallpaper='+ name,{
			headers: {
...
function pinterest(name){
	return new Promise(async(resolve,reject) => {
		 axios.get('https://id.pinterest.com/search/pins/?autologin=true&q=' + name, {
			headers: {
        ...
        const link = $(b).find('img').attr('src')
		 ...
		 search.push(v.replace(/236/g,'736'))
			})
			search.shift();
		resolve(search)
		})
